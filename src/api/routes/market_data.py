...
@router.get("/quote")
def get_stock_quote(symbol: str = Query(..., min_length=1, description="股票代码")):
    """获取个股实时行情"""
...
def get_financial_summary(symbol: str = Query(..., min_length=1)):
    """获取个股基本面摘要"""
...
@router.get("/industry")
def get_industry_stocks(industry: str = Query(..., min_length=1)):
    """按行业获取股票列表"""
    return fetch_industry_stocks(industry)


@router.get("/index-constituents")
def get_index_constituents(index: str = Query("csi300")):
    """获取指数成分股列表"""
