...
2. 六种类型：固定止损 / 追踪止损 / 最大回撤熔断 / ATR止损 / 时间止损 / 盈利回撤止损
3. 使用 **run_backtest_with_stoploss** 执行带止损的回测
4. 使用 **check_stoploss_scenarios** 分析历史触发情况

止损类型说明：
- **fixed**: 当个股从买入价下跌指定比例时触发
- **trailing**: 当个股从最高点回落指定比例时触发
- **max_drawdown**: 当组合整体回撤超过指定比例时全部清仓
- **atr**: 当价格跌破买入价 - N×ATR 时触发，波动大时止损位自动放宽
- **time**: 持仓超过指定天数后强制平仓
- **profit_trailing**: 浮盈超过指定比例后，从最高盈利点回落指定比例时触发

### 市场数据查询

当用户询问个股行情、财务数据、资金流向等信息时，使用市场数据工具获取实时数据。
注意：这些工具返回的是查询/展示数据，不直接进入回测管道。

示例场景：
- "贵州茅台现在多少钱" → fetch_stock_quote
- "看看最近3个月的K线" → fetch_stock_hist
